...
namespace TempestEngine
{
    ...
    public:
        ...
        static float dot(Vector2 from, Vector2 to)
        ...
        { // Returns a NEW vector. Example usage would be Vector2 Added = Vector2(2, 5, 1) + Vector(1, 0, 5) = Vector2(3, 5, 6)
            return Vector2(x + other.x, y + other.y);
        }
        Vector2 operator-(Vector2 other)
        { // Returns a NEW vector. Example usage would be Vector2 Subtracted = Vector2(2, 5, 1) - Vector(1, 0, 5) = Vector2(3, 5, 6)
            return Vector2(x - other.x, y - other.y);
        }
        Vector2 operator*(float scalar)
        { // Returns a NEW vector. Example usage would be Vector2 Scaled = Vector2(2, 5, 1) * 4 = Vector2(8, 20, 4)
            return Vector2(x * scalar, y * scalar);
        }
        void operator*=(float scalar)
        { // Modifies this vector to a certain scale. Example of this would be: Vector2 MovementVector *= 9.82f
            x = x * scalar;
            y = y * scalar;
        }
        void operator+=(Vector2 other)
        { // Modifies this vector. Example of this would be: Vector2 PushVector (it's downwards) += Vector2 GravitationalVector
            x, y += other.x, other.y;
        }
        void operator-=(Vector2 other)
        { // Modifies this vector. Example of this would be: Vector2 PushVector (it's sideways along a surface that has friction) -= Vector2 FrictionVector
            x, y -= other.x, other.y;
        }
        ...
        Vector2 operator()(float v, float w)
        {
            return Vector2(v, w);
        }
